...
function validateWebSDKUrl({
  ...
  if (query.version === undefined || !semverRegex.test(query.version)) {
    throw new Error(`Invalid version parameter on web-sdk bridge url: ${query.version}`);
  }
  let url = null;
  try {
    url = new URL(query.origin);
  } catch (error) {
    throw new Error(`Invalid origin parameter on web-sdk bridge url: ${query.origin}`);
  }
